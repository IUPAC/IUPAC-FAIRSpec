...
# 2026.03.18 FN -- adapted the bash script to run on MacOS (remove getopt, mapfile, readarray)
...
while [[ -n "$1" ]]; do
  ...
  case "$1" in
    ...
    -c | --clean)
        ...
        ;;
    -f | --file) 
        if [[ "$doiTodoFilePath" == "?" || -z "$doiTodoFilePath" ]]; then
            doiTodoFilePath="$2"
        fi
        shift 2
        ;;
...
# Processing the file todo list
if [[ -f "$doiTodoFilePath" ]]; then
    echo "$doiTodoFilePath exists"
    #Initalize the dois array
    if [[ "$dois" == "?" || -z "$dois" ]]; then
        dois=()
    fi
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$trimmed_line" == "STOP" ]]; then
            echo "Stopping at $trimmed_line"
            break
        fi   
        # Remove the comment section after each doi
