...
IFD.loadContents = function(aid) {
	...
	if (dc.spectra) {
		...
		s += "<a href=\"javascript:IFD.showAllSpectra('"+id+"')\">Spectra(" + items.length + ")</a>&nbsp;&nbsp;&nbsp;"
	}
	if (dc.structures) {
		var items = dItems["structures"] = IFD.getIDs(dc.structures);
		s += "<a href=\"javascript:IFD.showAllStructures('"+aid.id+"')\">Structures(" + items.length + ")</a>&nbsp;&nbsp;&nbsp;"
	}
	return s + "</td></tr>";
}
...
var addMapRows = function(name, map) {
	var s = "";
	var n = 0;
	for (var key in map) {
		if (n++ == 0 && name)
			s += "<tr><td><h4>" + name + "</h4></td></tr>";
		s += "<tr><td>" + key + "</td><td>" + map[key] + "</td></tr>";
	}
	return s;
}
...
addCompoundSpectrumRow = function(aidID, id) {
	...
	s += "</td></tr>";
	s += "<tr><td><table>";
		s += addMapRows("IFD&nbsp;Properties", spec.properties);
