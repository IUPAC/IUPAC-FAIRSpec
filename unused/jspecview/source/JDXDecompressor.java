...
public class JDXDecompressor {
	...
	/**
	 * Initialises the <code>JDXDecompressor</code> from the compressed data, the x
	 * factor, the y factor and the deltaX value
	 * 
	 * @param t       the data to be decompressed
	 * @param firstX  TODO
	 * @param xFactor the x factor
	 * @param yFactor the y factor
	 * @param deltaX  the delta X value
	 * @param nPoints the expected number of points
	 */
	public JDXDecompressor(JDXSourceStreamTokenizer t, double firstX, double xFactor, double yFactor, double deltaX,
			int nPoints, boolean isNTUPLE) {
		this.t = t;
	...
	public Coordinate[] decompressData(SB errorLog, double[] firstLastX) {
		...
		if (debugging)
			logError("firstX=" + firstX + " xFactor=" + xFactor + " yFactor=" + yFactor + " deltaX=" + deltaX
					+ " nPoints=" + nPoints);
		...
		try {
			while ((line = t.readLineTrimmed()) != null && line.indexOf("##") < 0) {
				...
				if (ipt == 0) {
					...
					// System.out.println(ipt + " " + xdif + " " + point + " " + dx + " " +
					// lastPoint);
					...
					if (isCheckPoint && xdif < difMax) {
						...
						if (xdif < dif06 || xdif > dif14)
							logError(lastLine + "\n" + line + "\nX-sequence Checkpoint Error! Line " + lineNumber
									+ " |x1-x0|=" + xdif + " instead of " + Math.abs(deltaX) + " for x1="
									+ point.getXVal() + " x0=" + lastPoint.getXVal());
					}
				}
		...
		if (nPoints != ipt) {
			...
			logError("Decompressor did not find " + nPoints + " points -- instead " + ipt + " xyCoords.length set to "
					+ n);
	...
	private void testAlgorithm() {
		/*
		 * line =
		 * "4265A8431K85L83L71K55P5j05k35k84k51j63n5K4M1j2j10j97k28j88j01j7K4or4k04k89";
		 * lineLen = line.length(); System.out.println(getValue(allDelim)); while (ich <
		 * lineLen) System.out.println(line.substring(0, ich) + "\n" + ipt++ + " " +
		 * (yval = getYValue())); ipt= 0;
		 */ }
