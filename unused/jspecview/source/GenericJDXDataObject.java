 ...
 * 
 */
public class GenericJDXDataObject extends JDXHeader {
  ...
  protected double maxX = Double.NaN, maxY = Double.NaN;
  protected double deltaX = Double.NaN;
  ...
  public void checkRequiredTokens() throws JSVException {
    String err = (fileFirstX == ERROR ? "##FIRSTX"
        : fileLastX == ERROR ? "##LASTX"
            : nPointsFile == -1 ? "##NPOINTS"
                : xFactor == ERROR ? "##XFACTOR"
                    : yFactor == ERROR ? "##YFACTOR" : null);
    if (err != null)
  ...
  protected double y2D = Double.NaN;
  ...
  protected String fixNucleus(String nuc) {
  ...
  /**
   * source:
   * http://www.mathworks.com/matlabcentral/fileexchange/11722-nmr-properties/content/isotopes.m
   * % REFERENCES % 1. "Nuclear spins, moments, and other data related to NMR
   * spectroscopy" (9-93) % CRC Handbook of Chemistry and Physics, 83th Ed., CRC
   * Press, Boca Raton, FL, % 2002. % 2. Stone, N. J.,
   * <www.nndc.bnl.gov/nndc/stone_moments/> % 3. http://www.hbcpnetbase.com/
   */
  ...
  protected final static double[] gyroData = { 1, 42.5774806, //*H   Hydrogen  1/2
  ...
  protected static double getGyroMagneticRatio(String nuc) {
    int pt = 0;
  ...
  public void copyTo(GenericJDXDataObject newObj) {
  ...
  protected boolean xIncreases = true;
  ...
  public double getMinX() {
    return (Double.isNaN(minX)
        ? (minX = Coordinate.getMinX(xyCoords, 0, xyCoords.length - 1))
        : minX);
  }
  ...
  public double getMinY() {
    return (Double.isNaN(minY)
        ? (minY = Coordinate.getMinY(xyCoords, 0, xyCoords.length - 1))
        : minY);
  }
  ...
  public double getMaxX() {
    return (Double.isNaN(maxX)
        ? (maxX = Coordinate.getMaxX(xyCoords, 0, xyCoords.length - 1))
        : maxX);
  }
  ...
  public double getMaxY() {
    return (Double.isNaN(maxY)
        ? (maxY = Coordinate.getMaxY(xyCoords, 0, xyCoords.length - 1))
        : maxY);
  }
  ...
  public void doNormalize(double max) {
    ...
    Logger.info(
        "Y values have been scaled by a factor of " + normalizationFactor);
  }
  ...
  public double getDeltaX() {
    return (Double.isNaN(deltaX)
        ? (deltaX = Coordinate.deltaX(getLastX(), getFirstX(), xyCoords.length))
        : deltaX);
  }
